...
import { ProductLists } from "../../data/Category";
import ImageGallery from "react-image-gallery";
import "react-image-gallery/styles/css/image-gallery.css";
import Modal from "../../components/RequestPrice/RequestPrice.js";

const ProductDetails = () => {
  const [isDefaultViewOpen, setDefaultViewOpen] = useState(false);
  const { id } = useParams();
  ...
  const ProductDetails = ProductLists.filter((ProdDet) => {
    if (ProdDet["id"] == id) return ProdDet;
  })[0];

  // Component Example
  const TabOne = () => {
    return (
      <div className="descriptions-sub-text">
        <Markup content={ProductDetails.description} />
      </div>
    );
  };
  const TabTwo = () => {
    return (
      <div className="descriptions-sub-text">
        <Markup content={ProductDetails.process} />
      </div>
    );
  };
  const TabThree = () => {
    return (
      <div className="descriptions-sub-text">
        <Markup content={ProductDetails.care} />
      </div>
    );
  };
  ...
  const virtualURL =
    "https://khenshu.s3.ap-south-1.amazonaws.com/products/Paintings_AR/" +
    ProductDetails.virtualRealityURL +
    ".html";
  return (
    <div className="pdp">
        ...
        <div className="row">
          <div className="col-xs-12 col-sm-12 col-md-6 col-lg-6 text-center">
            {!isDefaultViewOpen && (
              <div className="left-side">
                <ImageGallery
                  items={ProductDetails.imageUrl?.imageList}
                ...
                <button
                  ...
                  value="3D View"
                  onClick={() => setDefaultViewOpen(!isDefaultViewOpen)}
                >
            ...
            {isDefaultViewOpen && (
              <div className="left-side-1">
                <iframe src={virtualURL} width="100%" />
                <button
          ...
          <div className="col-xs-12 col-sm-12 col-md-6 col-lg-6">
            <div className="right-side">
              <div className="prod-title">{ProductDetails.title}</div>

              <div className="prod-sub-title">
                {ProductDetails.prodSubTitle}
              </div>
      ...
      {isModalOpen && (
        <Modal handler={toggleModal} prodName={ProductDetails.title} />
      )}
    </div>
  );
};
